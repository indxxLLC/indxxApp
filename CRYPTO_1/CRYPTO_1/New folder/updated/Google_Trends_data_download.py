...
DATA_FILES_PATH ="E:\\CRYPTO_1\\New folder\\Files\\Historical_Data\\"
todays_date=date.today()
MOD_CRYPTO_MASTER_LIST = pd.read_csv(filepath_or_buffer = DATA_FILES_PATH+"Universe_Masterlist\\" + "CryptoCoin_Masterlist"+str(todays_date)+".csv", encoding = "utf-8")
...
for i in range(0,len(kw_list)):
    if i > 0 :
        ...
        sub_frame = pytrends1.interest_over_time()
        sub_frame =  sub_frame.drop(['isPartial',kw_list[1]],axis=1)
...
for i in range(0,len(trend_data.columns)):    
    coin_file = pd.read_excel("E:\\CRYPTO_1\\New folder\\Files\\Historical_Data\\Prices\\" + FINAL_COIN_LIST_sort['COIN'][i] +".xlsx",sheet_name='Sheet1')
    ...
    coin_file_new.to_excel("E:\\CRYPTO_1\\New folder\\Files\\Historical_Data\\Prices\\" + FINAL_COIN_LIST_sort['COIN'][i] +".xlsx",sheet_name='Sheet1',index=False)
