...
from FUNCTIONS import reload_func as reload
...
if CALC_MODE == 'daily':
    ...
    CALC_START_DATE =  '2020-04-01' #START_DATE       #'2020-02-01' #2004-12-31'#
    CALC_END_DATE =  '2020-04-30' #END_DATE           #'2020-02-29'
...
stamp =  int(datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
#try:
        ...
        if CALC_DATE == BT_START_DATE :
            ...
            if IND4 :
                    ...
                    ################## RED SIGNAL  ######################
                        ...
                        #################  YELLOW SIGNAL ########################                            
                            TM1 = pd.DataFrame()
                            TM1 = dt.imp_Nth_date(CALENDAR_ID, 'LAST', 'B', '', CALC_DATE, 1)
...
#Exporting EXPOSURE
        ...
        #Sending the exposures file to ops.
#        EXPOSURE1 = [i for i in EXPOSURE if i >0.0]
        print('exported to f_calc for '+str(CALC_DATE))
        func.WEIGTHS_CSV([str("{0:.10f}".format(X*100)+"%") for X in EXPOSURE],CALC_DATE.strftime('%Y-%m-%d'))
        time.sleep(5.5)
        func.success_send_attach("E:\\EPIC_RUST\\FIRSTONE\\output\\TLLCGI_" +CALC_DATE.strftime('%Y-%m-%d')+".csv")
       
    else:
        print(CALC_DATE)
        func.error_msg('','Not a Calc date')
print(time.time() - ST)
        
#except Exception as e:
#    func.error_msg(e,'TLLCG')
