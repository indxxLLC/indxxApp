...
def index(request):
	#return HttpResponse(request.META.get('HTTP_X_FORWARDED_FOR'))
	# return HttpResponse(request.META.get('REMOTE_ADDR'))
	try:
		email = request.GET.get('email')
		if '@indxx.com' not in email:
	...
	request.session['is_authentication'] = True
	...
	if request.method == 'POST':
		...
		if(indexName == "" and cname1 == "" and month =="" and month2 =="" and (sel_type1!="")):
			...
			if(indexName != "" and cname1 != ""):
				...
				info = Registerindex.objects.filter(Client_Name = cname1).filter(lookups).order_by('id').reverse()
			elif(indexName != ""):
				lookups = Q(Index_Name__icontains = indexName)
								...
								for i in range(0,num_month_review):
									if(month1_review!=""):
										...
										if(month1_review<cal_month_review):
											print(ind.Index_Name,cal_month_review)
											year = year+1
											if(month1_review==0):
												month1_review=12
									if eff_rule_review:
										...
										print(ind.Index_Name, e_date_review)
