	...
	<?php //if(isset($_POST['period']) && isset($_POST['Step0']) ) { ?>
		...
		<div class="row">
			<div class="col-lg-12 col-xs-12">
				<ul class="Notice" >
					<li style="font-size:15px;list-style:none; font-weight:bold;">Note : 1. CSV file should be Period , ISIN , Weight, Start Date, End Date and Country format.</li>
					<li style="font-size:15px;list-style:none; font-weight:bold;">Note : 2. System will treat weight as in % only.</li>	
					<li style="font-size:15px;list-style:none; font-weight:bold;"><a href='inputs/BT Tool User Help.pdf' target="_blank">Download  User Help Guide</a></li>
<li style="font-size:15px;list-style:none; font-weight:bold;"><a href='inputs/inputsample.csv'>Download  sample Input file</a></li>
					<li style="font-size:15px;list-style:none; font-weight:bold;"><a href='inputs/Withholding-Tax-Rates.csv'>Download  Tax Rate Input file </a></li>						
				</ul>
			</div>
		</div>
		...
		<div class="row">
			<form name="frmStep1" id="frmStep1" action="processd2.php" method="post" enctype="multipart/form-data">		
				<div class="col-lg-3 col-xs-3"  style="width:35%;">	
					<!--
					<label >Period *</label> 	
					...
					<input type="hidden" id="SecuritiesStartDateJSON" name ="StartDate" value="" />					
					<input type="hidden" id="SecuritiesEndDateJSON" name ="EndDate" value="" />		
					<input type="hidden" id="SecuritiesPeriodsJSON" name ="period" value="" />	
					<input type="hidden" id="Securitiescountry" name ="Securitiescountry" value="" />
					
					<label class="error_CVSFile" style="display:none;color:#CA4B4B;">Upload Portfolio.</label> 
					<label >Tax Rate (Optional)</label> 	
					<input type="file" id="tCVSFile" name ="tCVSFile" class = "input-block-level CVSFile" />	
					<br/><br/>
					<label >Treatment for spin off adjustment*</label> 	<br>
					<label style="margin-right:10px;"><input type="radio" class="input-block-level" name="spinoff" value="0" checked> MarketCap Adjustment (Divisor Adjustment for Spin-off) </label >
					<label ><input type="radio" class="input-block-level" name="spinoff" value="1"> Equal Weight Adjustment (Share Adjustment for Spin-off)  </label >
				...
				<div class="col-lg-6 col-xs-6" id="div_period_with_dates">	
						...
						// echo $startDate = $_POST['StartDate'][$i-1];
...
<script src="http://204.80.90.133/backtest_engine/plugins/daterangepicker/daterangepicker.js"></script>
<!-- datepicker -->
<script src="http://204.80.90.133/backtest_engine/plugins/datepicker/bootstrap-datepicker.js"></script>
  <script> 
				...
				success: function (data) {					
					...
					if(obj.success) {
						...
						if(obj.message)  {
							...
							// console.log(obj.SecuritiesEndDateJSON);
							//var SecuritiesWeightJSON = jQuery.parseJSON(obj.SecuritiesWeightJSON);
							
							$("#SecuritiesWeightJSON").val(obj.SecuritiesWeightJSON);
							$("#Securitiescountry").val(obj.Securitiescountry);
							$("#SecuritiesStartDateJSON").val(obj.SecuritiesStartDateJSON);
							$("#SecuritiesEndDateJSON").val(obj.SecuritiesEndDateJSON);
							$("#SecuritiesPeriodsJSON").val(obj.Periods);
								
							
							// var Thedates = jQuery.parseJSON(obj.thedates);
							// if(Thedates[0]){
								// $( "#StartDate_1").val(Thedates[0]);
							// }
							...
							for (i = 0; i < Thedates.length; i++) {
								...
								if(i<Thedates.length) {									
									$( "#EndDate_"+x).val(Thedates[i]);
								}
							}
							*/
