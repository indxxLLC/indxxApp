<?php
		...
		if($_FILES["CVSFile"]["name"]) { 
			...
			//if("text/plain" === trim($mime)  && trim(strtolower($ext)) == "csv") {
				...
				if($periodNull || $SecuritiesWeightNull) {
					$message .= " <li style='list-style:none;'>Error	:	Few Securities does not have proper period or proper weight. Please check your portfolio.</li>";
	...
	if($file)
		$returnArr = array("message"=>$message,"warrning"=>$warrning,"success"=>$success,"thedates"=>$checkResultDates,"file"=>$file,"warrningNoHTML"=>$warrningNoHTML,"Securities" => $Securities,"SecuritiesWeightJSON" => $SecuritiesWeightJSON);
	else
		$returnArr = array("message"=>$message,"warrning"=>$warrning,"success"=>$success,"file"=>"","thedates"=>$checkResultDates,"warrningNoHTML"=>$warrningNoHTML,"Securities" => $Securities,"SecuritiesWeightJSON" => $SecuritiesWeightJSON);
