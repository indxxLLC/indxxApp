		...
		if($_FILES["CVSFile"]["name"]) { 
			...
			//if("text/plain" === trim($mime)  && trim(strtolower($ext)) == "csv") {
				...
				while (($line = fgetcsv($Readfile)) !== FALSE) {
					...
					if($totalLines>=1) {	
						...
						if(isset($line[2])) {
							if(!is_numeric($line[2])) {
								...
								if($line[2]){
								$line[2] = number_format($line[2],20);
								} else {
					...
					if(!isset($line[2])  || !is_numeric($line[2])  ) {
							$SecuritiesWeightNull = true;
							break;
						}
					}
				   ...
				   if($totalLines==1) {									
					...
					if( $tperiodNo >= 0  && $line[2]>0 && !in_array($line[1],$SecuritiesUniqueISNs[$tperiodNo])) {						
						...
						$SecuritiesWeight[$tperiodNo][$line[1]]= round($line[2],20);
						$Securitiescountry[$tperiodNo][$line[1]]=$line[5];
					}
					else if(in_array($line[1],$SecuritiesUniqueISNs[$tperiodNo])){
						$message .="Peroid -".($tperiodNo+1)." ISIN Repeated-".$line[1];
						$SecuritiesWeightNull=true;
					}
					/*if($tperiodNo>=0 && $line[2]>0  && in_array($line[1],$SecuritiesUniqueISNs[$tperiodNo])) {
						//$message.= $SecuritiesWeight[$tperiodNo][$line[1]];
						
						$SecuritiesWeight[$tperiodNo][$line[1]]= $SecuritiesWeight[$tperiodNo][$line[1]]+round($line[2],20);
						
						// $SecuritiesStartDate[$tperiodNo][$line[1]]= $line[3];
						// $SecuritiesEndDate[$tperiodNo][$line[1]]= $line[4];
				...
				if($periodNull || $SecuritiesWeightNull || $SecuritiesStartDateNull) {
					$message .= " <li style='list-style:none;'>Error	:	Few Securities does not have proper period, proper weight or proper Start Date and End Date. Please check your portfolio.</li>";
					...
					for($outer= 0; $outer < $overAllPeriods; $outer++) { 				
							...
							for($We=$PeriodBreakPoints[$outer]; $We < $PeriodBreakPoints[$outer+1]; $We++) {
								...
								$weight_with_seven_points = number_format($file_contents[$We][2],20);
...
$qry = "select a.fsym_id, a.p_price, a.p_date ,a.p_date as date, b.isin";
			...
			$qry .= " WHERE b.isin in  (". "'" . implode ( "', '", array_values($SecuritiesUniqueISNs[$outer]) ) . "'".")  AND  a.p_date between '".date('Y-m-d', strtotime("-5 day", strtotime($SecuritiesStartDate1[$outer])))."' and '".date('Y-m-d', strtotime($SecuritiesStartDate1[$outer])) ."'";						
