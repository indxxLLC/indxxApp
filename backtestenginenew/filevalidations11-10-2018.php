	...
	// if(isset($_POST['period']) && $_POST['period'] > 0 ) {		
		...
		if($_FILES["CVSFile"]["name"]) { 
			...
			//if("text/plain" === trim($mime)  && trim(strtolower($ext)) == "csv") {
				...
				$period_no = 0;
				for($i==0; $i<=count($tperiodNo); $i++){
					if(count($SecuritiesUniqueISNs[$i]) !== count(array_unique($SecuritiesUniqueISNs[$i])))
					{
						$period_no = $i;
						$dupliacte_ISIN = true;
					}
				...
				if($dupliacte_ISIN){
					$message .= " <li style='list-style:none;'>Error : There are some duplicate ISIN in $period_no period.</li>";
					$returnArr = array("message"=>$message,"warrning"=>"","success"=>"true","file"=>"","warrningNoHTML"=>"");		
					echo JSON_encode($returnArr);
					exit();						
				}elseif($periodNull || $SecuritiesStartDateNull) { //$SecuritiesWeightNull ||
