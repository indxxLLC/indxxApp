	...
	<div class="container main-body">	
		...
		for($p=0;$p < count($TmpInvestmentValuesPk);$p++) {
			foreach($TmpInvestmentValuesPk[$p] as $keydts => $DateValue) {
				...
				$InvestmentValuesPk[$p][$keydts] = round($sum,7,PHP_ROUND_HALF_UP); 
			}
		...
		for($p=0;$p < count($TmpDivisorPk);$p++) {
			...
			foreach($TmpDivisorPk[$p] as $keydts => $DateValue) {
				...
				$tmpSumOfSharesDividend[$p][$keydts] 	= round($sum1,7,PHP_ROUND_HALF_UP); 
				$tmpSumOfSharesPrice[$p][$keydts] 		= round($sum2,7,PHP_ROUND_HALF_UP);
				$tmpSumOfSharesDividendUSD[$p][$keydts] = round($sum3,7,PHP_ROUND_HALF_UP);
		...
		for($p=0;$p < count($tmpSumOfSharesPrice);$p++) {
			...
			foreach($tmpSumOfSharesPrice[$p] as $keydts => $DateValue) {							 
				...
				if($count>=2) {
					...
					$dd 	= $tmpSumOfSharesDividendUSD[$p][$keydts] / $IndexValueP[$p][$daybefore];				
					
					$DivisorP[$p][$keydts] = $DivisorP[$p][$daybefore] - $a - $c ;
					$DivisorT[$p][$keydts] = $DivisorT[$p][$daybefore] - $dd - $c ;					
					...
					$DivisorP[$p][$keydts] 		= round($DivisorP[$p][$keydts],7,PHP_ROUND_HALF_UP);
					$DivisorT[$p][$keydts] 		= round($DivisorT[$p][$keydts],7,PHP_ROUND_HALF_UP);					
					
					
					if($DivisorP[$p][$keydts]>0) {
						
						$d = $InvestmentValuesPk[$p][$keydts] / $DivisorP[$p][$keydts] ; 						
						$IndexValueP[$p][$keydts]  	= round( $d ,7,PHP_ROUND_HALF_UP);
					...
					if($DivisorT[$p][$keydts]>0) {
						...
						$IndexValueT[$p][$keydts]  	= round( $ddd ,7,PHP_ROUND_HALF_UP);
