			...
			foreach($DataTmpPrice[$i] as $IsnKey =>$data) {
				...
				while($Startdate!=$EndDate) {
					if (array_key_exists($Startdate,$DataTmpPrice[$i][$IsnKey])) {
						...
						$FinalPriceData[$i][$IsnKey][$Startdate]['newChangedPriceUSD'] = round( $FinalPriceData[$i][$IsnKey][$Startdate]['price'] /  $FinalPriceData[$i][$IsnKey][$Startdate]['exchangeRate'],4, PHP_ROUND_HALF_UP);
