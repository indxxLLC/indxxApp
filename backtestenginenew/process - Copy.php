...
?>
	<div class="container main-body">	
		...
		$header = array($titledate,$titleindexpr,$titleindextr);
		ob_start();
		fputcsv ($file, $header);		
		$html = '';
		for($i=0;$i<count( $IndexValueP);$i++) {
			...
			<div class="row">
					...
					<div class="col-lg-12 col-xs-12"><label> Period <?php echo $prdshow?> start date <?php echo$StartDate?> and End date <?php echo$EndDate?></label></div>
			</div>
			<div class="row">
		...
		<?php
			foreach($IndexValueP[$i] as $Date =>$indexValue) {
				...
				$fields = array($date,$indexValueP,$indexValueT);
				fputcsv($file,$fields);
			?>
			<div class="row">
			<div class="col-lg-1 col-xs-1"><?php echo $srNo?></div>					
			<div class="col-lg-3 col-xs-3"><?php echo $adate?></div>
			<div class="col-lg-4 col-xs-4"><?php echo$indexValue?></div>
			<div class="col-lg-4 col-xs-4"><?php echo $indexValueTR?></div>				
			</div>
		<?php		}
		}
